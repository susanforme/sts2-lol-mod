...
| `JHIN-DANCING_GRENADE` | 曼舞手雷 | Dancing Grenade | 攻击 | `1` | 普通 | 单体敌人 | `src/cards/DancingGrenade.cs` | 造成伤害；若击杀目标，会随机弹跳到另一名敌人并提高伤害。 |
...
| `JHIN-RELOAD` | 装填 | Reload | 技能 | `0` | 普通 | 自身 | `src/cards/Reload.cs` | 将子弹装填至 4 并抽牌；本回合禁用华彩，使用后消耗。 |
...
| `JHIN-WHISPER_VOLLEY` | 低语齐射 | Whisper Volley | 攻击 / 射击 | `2` | 罕见 | 全体敌人 | `src/cards/WhisperVolley.cs` | 对所有敌人进行射击；华彩时再追加一轮全体伤害。 |
...
| `JHIN-THIRD_GUNSHOT` | 第三声枪响 | Third Gunshot | 攻击 / 射击 | `1` | 罕见 | 单体敌人 | `src/cards/ThirdGunshot.cs` | 只能在子弹为 2 时使用；射击造成伤害，华彩时回能。 |
...
| `JHIN-FATEMAKER` | 戏命师 | Fatemaker | 能力 | `2` | 稀有 | 自身 | `src/cards/Fatemaker.cs` | 提高子弹上限，并在每回合开始时获得能量。 |
